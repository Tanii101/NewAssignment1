...
int main(int argc, char *argv[]) {
    ...
    char *coords_file = argv[1];
    char *output_file = argv[2];

    int num_coords = readNumOfCoords((char *)coords_file);
    if (num_coords < 0) {
        return EXIT_FAILURE;
    }

    double **coords = readCoords((char *)coords_file, num_coords);
    if (coords == NULL) {
        return EXIT_FAILURE;
    }
    ...
    writeTourToFile(tour, num_coords + 1, (char *)output_file);
